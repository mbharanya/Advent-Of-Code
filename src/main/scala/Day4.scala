...
object Day4 {
  ...
  sealed trait Event
  case class BeginShift(guardNo: Int) extends Event
  ...
  def parseRecords(input: List[String]): List[Record] = input.map(parseRecord).sortBy(_.timestamp)
  ...
  trait Strategy {
    def chooseShifts(shifts: List[Shift]): Int
    def choose(input: List[String]): Int = chooseShifts(parseShifts(parseRecords(input)))
  }
  ...
  object Strategy2 extends Strategy {
    override def chooseShifts(shifts: List[Shift]): Int = {
      val guardSleeps = shifts.groupBy(_.guard).mapValues(_.map(_.sleep.toSeq).reduce(_ ++ _))
  ...
  lazy val input = Source.fromFile(filename).getLines.toList
